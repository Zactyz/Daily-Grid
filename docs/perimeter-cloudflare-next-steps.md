...
4. **Validate the share wiring** by invoking the shared helper. For example:
   ...
   import { buildShareText } from './games/common/share.js';
   console.log(buildShareText({ gameName: 'Perimeter' }));
   NODE
   ```
   The text should read:
   ```
   I just completed the daily Perimeter puzzle on Daily Grid. Can you beat my time?
   ```
   The UI then calls `shareWithFallback`/`showShareFeedback` so mobile browsers trigger `navigator.share` and desktop users fall back to clipboard. Share payloads intentionally omit URLs and rely on the share image + text.
...
4. **UI sanity check** – load `/games/perimeter/`, solve a grid, and make sure the timer/completion modal appears, the leaderboard modal fills with the latest scores, the claim initials form shows/hides when allowed, and the share button builds the text above and triggers `shareWithFallback` with a success or clipboard fallback UI state.
5. **Monitor persistence** – confirm the leaderboard always reads from `perimeter_scores`, the 10‑minute initials window behaves the same as in Snake/Pathways, and share payloads stay URL-free.
