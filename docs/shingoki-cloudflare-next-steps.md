# Shingoki Cloudflare release + testing notes
...
1. **Start the Pages dev server with the shared bindings** so the functions see the same D1 + KV as prod:
   ...
   ```
2. **Seed the local D1 schema** before hitting the APIs. The first time you run the previous command, Miniflare will create `tmp/miniflare-dev-state/v3/d1/miniflare-D1DatabaseObject/<hash>.sqlite`. Run the init script against that file (or rerun `npx wrangler d1 execute ... --local --persist-to=./tmp/miniflare-dev-state --file=./scripts/init-db.sql` **before** the dev server starts) so `shingoki_scores` + indexes exist when the worker boots. If you already started the server and it created a different `<hash>.sqlite`, apply `scripts/init-db.sql` directly against every `.sqlite` file in `tmp/miniflare-dev-state/v3/d1/miniflare-D1DatabaseObject`.
3. **Smoke-test the API surface** against `http://localhost:8787`:
   ```bash
   curl -s -X POST http://localhost:8787/api/shingoki/complete -H 'Content-Type: application/json' -d '{"puzzleId":"YYYY-MM-DD","anonId":"<uuid>","timeMs":90000,"hintsUsed":0}'
   curl -s "http://localhost:8787/api/shingoki/leaderboard?puzzleId=YYYY-MM-DD"
   curl -s -X POST http://localhost:8787/api/shingoki/claim-initials -H 'Content-Type: application/json' -d '{"puzzleId":"YYYY-MM-DD","anonId":"<same uuid>","initials":"ZAC"}'
   ```
...
4. **Validate the share wiring** by invoking the shared helper. For example:
   ...
   ```
   Shingoki by Daily Grid
   Wed, Feb 4, 2026 • 7×7
   Time: 1:38

   https://dailygrid.app/games/shingoki/
   ```
...
1. **Apply the schema** – run `npm run db:init` so the production D1 (`daily-grid-db`) gains `shingoki_scores` + indexes every time `scripts/init-db.sql` changes.
...
4. **UI sanity check** – load `/games/shingoki/`, solve a grid, and make sure the timer/completion modal appears, the leaderboard modal fills with the latest scores, the claim initials form shows/hides when rank ≤ 10, and the share button builds the text above and triggers `shareWithFallback` with a success or clipboard fallback UI state.
5. **Monitor persistence** – confirm the leaderboard always reads from `shingoki_scores`, the 10‑minute initials window behaves the same as in Snake/Snake/Pathways, and the share URL stays `https://dailygrid.app/games/shingoki/` (or the staging equivalent).
