...
export async function onRequest(context) {
  ...
  try {
    ...
    const result = await claimInitials(env.DB, 'hashi_scores', puzzleId, anonId, initials);
    if (result.error) return jsonError(result.error, result.status);
    return jsonOk(result);
  } catch (err) {
    return internalError(err, 'Hashi claim-initials');
  }
}
