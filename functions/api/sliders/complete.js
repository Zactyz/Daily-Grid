...
export async function onRequest(context) {
  ...
  try {
    ...
    const result = await submitAndRank(env.DB, 'sliders_scores', puzzleId, anonId, timeMs, hintsUsed);
    return jsonOk({ success: true, ...result });
  } catch (err) {
    return internalError(err, 'Sliders complete');
  }
}
