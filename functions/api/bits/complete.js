// POST /api/bits/complete - Submit completion time
...
export async function onRequest(context) {
  ...
  try {
    ...
    const existingScore = await env.DB.prepare(`
      SELECT time_ms FROM bits_scores
    ...
    if (existingScore) {
      ...
      await env.DB.prepare(`
        INSERT INTO bits_scores (puzzle_id, anon_id, time_ms, hints_used)
        VALUES (?1, ?2, ?3, ?4)
    ...
    console.error('Bits complete API error:', error);
