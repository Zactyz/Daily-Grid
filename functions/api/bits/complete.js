...
export async function onRequest(context) {
  ...
  try {
    ...
    const result = await submitAndRank(env.DB, 'bits_scores', puzzleId, anonId, timeMs, hintsUsed, userId);
    return jsonOk({ success: true, ...result });
  } catch (err) {
    return internalError(err, 'Bits complete');
  }
}
