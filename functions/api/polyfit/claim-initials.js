// POST /api/polyfit/claim-initials
...
export async function onRequest(context) {
  ...
  try {
    ...
    const result = await claimInitials(env.DB, 'polyfit_scores', puzzleId, anonId, initials);
    if (result.error) return jsonError(result.error, result.status);
    return jsonOk(result);
  } catch (err) {
    return internalError(err, 'Polyfit claim-initials');
  }
}
