// POST /api/shikaku/complete - Submit completion time
...
export async function onRequest(context) {
  ...
  try {
    ...
    if (!puzzleId || !/^[0-9]{4}-[0-9]{2}-[0-9]{2}$/.test(puzzleId)) {
    ...
    const existingScore = await env.DB.prepare(`
      SELECT time_ms FROM shikaku_scores
    ...
    if (existingScore) {
      ...
      await env.DB.prepare(`
        INSERT INTO shikaku_scores (puzzle_id, anon_id, time_ms, hints_used)
        VALUES (?1, ?2, ?3, ?4)
    ...
    console.error('Shikaku complete API error:', error);
