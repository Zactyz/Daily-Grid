// POST /api/pathways/claim-initials
...
export async function onRequest(context) {
  ...
  try {
    ...
    const result = await claimInitials(env.DB, 'pathways_scores', puzzleId, anonId, initials);
    if (result.error) return jsonError(result.error, result.status);
    return jsonOk(result);
  } catch (err) {
    return internalError(err, 'Pathways claim-initials');
  }
}
