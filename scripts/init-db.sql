...
-- Shingoki game leaderboard table
CREATE TABLE IF NOT EXISTS shingoki_scores (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  puzzle_id TEXT NOT NULL,
  anon_id TEXT NOT NULL,
  time_ms INTEGER NOT NULL,
...
-- Indexes for efficient queries
CREATE INDEX IF NOT EXISTS idx_shingoki_puzzle_time 
  ON shingoki_scores(puzzle_id, time_ms);

CREATE INDEX IF NOT EXISTS idx_shingoki_puzzle_created 
  ON shingoki_scores(puzzle_id, created_at);

CREATE INDEX IF NOT EXISTS idx_shingoki_puzzle_anon 
  ON shingoki_scores(puzzle_id, anon_id);

-- Shikaku game leaderboard table
